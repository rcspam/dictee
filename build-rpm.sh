...
prepare_buildroot() {
    ...
    cp "$PKG_DIR/usr/bin/dictee-diarize-llm" "$buildroot/usr/bin/"
    ...
    sed -i '1s|^#!/usr/bin/env python3|#!/usr/bin/python3|' \
        ...
        "$buildroot/usr/bin/dictee-diarize-llm" \
...
for uid in \$(loginctl list-sessions --no-legend 2>/dev/null | awk '{print \$2}' | sort -u); do
    ...
    \$_run systemctl --user preset dictee dictee-vosk dictee-whisper dictee-whisper-rust dictee-canary dictee-nemotron dictee-kyutai 2>/dev/null || true
...
if [ "\$1" -eq 0 ]; then
    ...
    for uid in \$(loginctl list-sessions --no-legend 2>/dev/null | awk '{print \$2}' | sort -u); do
        ...
        [ -d "/run/user/\$uid" ] || continue
        _run="sudo -u \$user XDG_RUNTIME_DIR=/run/user/\$uid DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/\$uid/bus"
        \$_run systemctl --user stop dictee-ptt dictee-tray dotoold dictee dictee-vosk dictee-whisper dictee-whisper-rust dictee-canary dictee-nemotron dictee-kyutai 2>/dev/null || true
        \$_run systemctl --user disable dictee-ptt dictee-tray dotoold dictee dictee-vosk dictee-whisper dictee-whisper-rust dictee-canary dictee-nemotron dictee-kyutai 2>/dev/null || true
        \$_run systemctl --user daemon-reload 2>/dev/null || true
    done
    # Clean locales
