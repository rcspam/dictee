...
// Parakeet-TDT sometimes decodes a whole chunk with ZERO punctuation, and
// grouping on .?! alone then collapses the entire chunk into one segment. The
// chunked pipeline dedups by segment midpoint inside a half-overlap zone of
// 37.5 s, so segments MUST stay well below that or overlap text gets
// duplicated. Split at a clear pause once the sentence is already abnormally
// long, and cap the duration as a backstop. Healthy punctuated sentences end
// on .?! long before 10 s and are unaffected. (Ported from master 6352b31.)
