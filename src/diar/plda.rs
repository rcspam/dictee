...
use crate::diar::linalg::{LinalgError, eigh_gen, inv};
use crate::diar::utils::l2_normalize_rows_f64;
...
impl PldaTransform {
    pub fn from_dir(models_dir: &Path) -> Result<Self, PldaError> {
        ...
        let precision_matrix = inv(&raw_transform.t().dot(&raw_transform))?;
        ...
        let between_class_covariance = inv(&tr_over_psi.dot(&raw_transform))?;

        // Generalized symmetric-definite eigenproblem, ascending eigenvalues
        // (LAPACK *sygv semantics — see diar::linalg::eigh_gen).
        let (eigenvalues, eigenvectors) = eigh_gen(&between_class_covariance, &precision_matrix)?;
...
mod tests {
    ...
    fn fixture_models_dir() -> std::path::PathBuf {
        Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/diar-fixtures/models")
    }

    #[test]
    fn transform_from_models_has_expected_shapes() {
        let plda = PldaTransform::from_dir(&fixture_models_dir()).unwrap();
