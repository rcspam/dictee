...
use crate::diar::utils::l2_normalize_rows;

#[derive(Debug, Clone, Copy)]
pub struct AhcConfig {
    pub threshold: f32,
    /// When set, cut the dendrogram to exactly this many clusters (clamped to
    /// the number of observations) instead of cutting at `threshold`. This is
    /// the "known number of speakers" (oracle) mode of pyannote pipelines.
...
pub fn cluster(embeddings: &ArrayView2<f32>, config: AhcConfig) -> Vec<usize> {
    ...
    match config.num_clusters {
        Some(target) => flat_clusters_by_count(observations, dendrogram.steps(), target),
        None => flat_clusters(observations, dendrogram.steps(), config.threshold),
    }
}

/// Cut the dendrogram to exactly `target` clusters by applying merge steps in
/// order until only `target` clusters remain. Labels follow first-seen leaf
/// order, matching the leader-order style of the threshold cut.
fn flat_clusters_by_count(observations: usize, steps: &[Step<f32>], target: usize) -> Vec<usize> {
    if observations == 0 {
        return Vec::new();
    }
    let target = target.clamp(1, observations);
    let merges_to_apply = observations - target;

    let total_nodes = observations + steps.len();
    let mut parent: Vec<usize> = (0..total_nodes).collect();
    for (step_idx, step) in steps.iter().take(merges_to_apply).enumerate() {
        let node_idx = observations + step_idx;
        parent[step.cluster1] = node_idx;
        parent[step.cluster2] = node_idx;
    }

    let mut root_label = std::collections::HashMap::new();
    let mut labels = vec![usize::MAX; observations];
    for leaf in 0..observations {
        let mut node = leaf;
        while parent[node] != node {
            node = parent[node];
        }
        let next = root_label.len();
        labels[leaf] = *root_label.entry(node).or_insert(next);
    }
    labels
}
...
mod tests {
    ...
    #[test]
    fn count_cut_clamps_target() {
        let embeddings = array![[1.0, 0.0], [0.95, 0.05], [-1.0, 0.0],];

        // target above observation count: every leaf keeps its own label
        let labels = cluster(
            &embeddings.view(),
            AhcConfig {
                threshold: 0.6,
        ...
        // target zero clamps to a single cluster
        let labels = cluster(
            &embeddings.view(),
            AhcConfig {
                threshold: 0.6,
    ...
    #[test]
    fn count_cut_single_cluster_merges_all() {
        let embeddings = array![[1.0, 0.0], [0.9, 0.3], [0.0, 1.0], [0.05, 1.0],];
