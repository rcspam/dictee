...
def save_config(backend, lang_source, lang_target, clipboard=True, animation="speech",
    ...
    _target = conf_path or CONF_PATH
    os.makedirs(os.path.dirname(_target), exist_ok=True)
    _tmp_fd, _tmp_path = _tmpmod.mkstemp(dir=os.path.dirname(_target), prefix=".dictee.conf.")
    try:
...
class DicteeSetupDialog(QDialog):
    def __init__(self, wizard=False, open_postprocess=False):
        ...
        # En mode wizard, écrire dans un fichier temporaire puis copier à Finish
        if self.wizard_mode:
            self._wizard_conf_path = CONF_PATH + ".wizard"
        else:
            self._wizard_conf_path = None
    ...
    def _wizard_next(self):
        ...
        if idx == self.stack.count() - 1:
            ...
            if idx + 1 == self.stack.count() - 1:
                # Entering checks page — save temp conf to real path for services,
                # start services, then verify. If wizard is cancelled, closeEvent cleans up.
                if self._wizard_conf_path and os.path.exists(self._wizard_conf_path):
                    os.replace(self._wizard_conf_path, CONF_PATH)
                    self._wizard_conf_path = None  # déjà copié
                self._on_apply()
    ...
    def closeEvent(self, event):
        ...
        if self.wizard_mode and not self._wizard_finished:
            # Nettoyer le fichier conf temporaire du wizard si annulé
            if self._wizard_conf_path and os.path.exists(self._wizard_conf_path):
                try:
                    os.unlink(self._wizard_conf_path)
                except OSError:
                    pass
            # Si la conf a été copiée vers CONF_PATH (page checks) mais wizard
            # pas terminé, supprimer aussi le vrai fichier
            if not self._wizard_conf_path and os.path.exists(CONF_PATH):
                # Le tmp a déjà été renommé en CONF_PATH — annulation après page checks
                try:
                    os.unlink(CONF_PATH)
                    # Arrêter les services démarrés par _on_apply
    ...
    def _on_apply(self):
        ...
        save_config(backend, lang_src, lang_tgt, clipboard, animation,
                    ...
                    conf_path=self._wizard_conf_path)
