...
class InstallThread(QThread):
    # done(ok, message). On success the message is empty after an install, or
    # carries the "nothing to do" text when the installed version is already
    # the published one.
    done = Signal(bool, str)
    progress = Signal(str)

    def __init__(self, installed_version="", parent=None):
        super().__init__(parent)
        # Empty when animation-speech is absent, or when no package manager
        # nor the zipapp would say which version is installed.
    ...
    def run(self):
        ...
        try:
            ...
            # Nothing to download when the published version is the one
            # already on disk. Only skip on a clear match: an unreadable
            # local version means we reinstall rather than stay silent.
            published = (release.get("tag_name") or "").strip().lstrip("vV")
            if (self.installed_version and published
                    and DicteeSetupDialog._parse_version(published)
                    <= DicteeSetupDialog._parse_version(self.installed_version)):
                self.done.emit(True, _("animation-speech {version} is already "
                                       "the latest release.").format(
...
class DicteeSetupDialog(QDialog):
    ...
    def _build_page_meeting(self):
        ...
        layout.addWidget(QLabel(
            _("Live meeting captures audio continuously and hands it off to the "
              "dictation window for transcription + diarization + LLM analysis "
              "when you stop it.")
        ))
    ...
    def closeEvent(self, event):
        ...
        # audit fix split _test_thread into dictee/translate-specific handles.
        for _attr in ("_test_dictee_thread", "_test_translate_thread"):
            _t = getattr(self, _attr, None)
            if _t and _t.isRunning():
                _t.stop()
                _t.wait(2000)
    ...
    @staticmethod
    def _animation_speech_version():
        """Installed animation-speech version, or "" when it cannot be told.

        Each package manager only answers on the distro that did the install,
        and a tarball install answers to none of them, so fall back to the
        zipapp, which carries __version__ wherever it came from. Without this
        the version was read through dpkg-query alone and every non-Debian
        user was simply told "installed".
        """
        for cmd in (["dpkg-query", "-W", "-f", "${Version}", "animation-speech"],
                    ["rpm", "-q", "--qf", "%{VERSION}", "animation-speech"],
                    ["pacman", "-Q", "animation-speech"]):
            try:
                r = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
            except (OSError, subprocess.SubprocessError):
                continue
            if r.returncode == 0 and r.stdout.strip():
                # pacman answers "name version"; deb and rpm carry a packaging
                # release suffix we do not compare on.
                return r.stdout.strip().split()[-1].split("-")[0]

        app = shutil.which("animation-speech")
        if app:
            try:
                import zipfile
                with zipfile.ZipFile(app) as z:
                    src = z.read("animation_speech/__init__.py").decode()
                m = re.search(r"__version__\s*=\s*['\"]([^'\"]+)", src)
                if m:
                    return m.group(1)
            except Exception:  # noqa: BLE001 — not a zipapp, unreadable, …
                pass
        return ""
    ...
    def _check_animation_speech(self):
        ...
        if has_anim:
            v = self._animation_speech_version()
            if v:
            ...
            # Keep the button: hiding it as soon as the binary existed left
            # everyone stuck on whatever version they first installed, with
            # no way to pick up a fix from here.
            self.btn_install_anim.setText(_("Check for animation-speech updates"))
            ...
            self.btn_install_anim.setVisible(True)
            # Re-enabled here so a failed attempt leaves a usable button.
            self.btn_install_anim.setEnabled(self.de_type != "gnome")
    ...
    def _on_install_animation(self):
        ...
        self._install_thread = InstallThread(self._animation_speech_version())
    ...
    def _on_install_finished(self, success, message):
        ...
        if success:
            self._check_animation_speech()
            if message:
                # Nothing was downloaded: already on the published version.
                QMessageBox.information(self, _("animation-speech"), message)
        else:
            self._check_animation_speech()
