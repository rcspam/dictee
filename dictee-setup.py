...
class DicteeSetupDialog(QDialog):
    ...
    def _on_apply(self):
        ...
        save_config(backend, lang_src, lang_tgt, clipboard, animation, ollama_model, ollama_cpu, trans_engine, lt_port)

        # Services systemd
        for svc, chk in [("dictee", self.chk_daemon), ("dictee-tray", self.chk_tray)]:
            action = "enable" if chk.isChecked() else "disable"
            subprocess.run(
