...
for svc in dictee dictee-tray dictee-ptt dotoold dictee-vosk dictee-whisper dictee-whisper-rust dictee-canary dictee-nemotron dictee-kyutai; do
    if [[ -n "${SUDO_USER:-}" ]]; then
...
if [[ $TARBALL_INSTALL -eq 1 ]]; then
    ...
    for bin in transcribe transcribe-daemon transcribe-client transcribe-diarize \
               ...
               dictee-stream dictee-meeting-live dictee-cheatsheet dictee-app-capture \
