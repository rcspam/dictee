...
EOF
    ...
    # nvcc 12.8 is REQUIRED on the build host: a 595.x driver is CUDA 13.2 and
    # rejects the PTX 9.3 that nvcc 13.3 emits. Override CUDA_LOCAL if the
    # toolkit lives elsewhere. (The old default pointed at the untracked POC
    # tree tests/poc-kyutai/cuda-12.8-local, which no longer exists.)
