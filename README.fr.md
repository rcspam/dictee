...
- **Durée de la dictée live** : la transcription de fichiers n'a **aucune limite de durée** — `dictee-transcribe` découpe n'importe quelle longueur (keynote de 54 min diarisée en ~122 s sur 8 Go). La **dictée live continue** (push-to-talk maintenu) est plafonnée par backend — Parakeet ~4 min 30, Canary ~2 min 30 ; Whisper et Vosk sans limite. Au-delà, l'enregistrement est sauvegardé et ouvert dans `dictee-transcribe`. La v1.4 prévoit d'étendre le pipeline découpé à la dictée live. → [Wiki Diarization](https://github.com/rcspam/dictee/wiki/Diarization)
