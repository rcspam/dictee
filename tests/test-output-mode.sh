...
for fn in _emit_sanitize _paste_text emit_text type_text \
          apply_continuation save_last_word use_streaming; do
...
echo "── garde streaming (master): modes non-type => batch"
DICTEE_STREAMING=true
DICTEE_ASR_BACKEND=nemotron
DIARIZE=false
TRANSLATE=false
DICTEE_STREAMING_FINAL_PASS=true
OUTPUT_MODE="type"
use_streaming && r=stream || r=batch
check "type: streaming autorisé" "$r" "stream"
OUTPUT_MODE="paste"
use_streaming && r=stream || r=batch
check "paste: streaming coupé (retombe en batch)" "$r" "batch"
OUTPUT_MODE="clipboard"
use_streaming && r=stream || r=batch
check "clipboard: streaming coupé (retombe en batch)" "$r" "batch"
